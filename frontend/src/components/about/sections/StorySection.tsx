          ...
          <ul className="space-y-4">
            {aboutTimeline.map((item, index) => (
              ...
              >
                ...
                <div>
                  <p className="text-sm uppercase tracking-[0.3em] text-muted-foreground">{item.year}</p>
                  <p className="text-sm text-muted-foreground mt-2 group-hover:text-foreground transition-colors duration-300">
                    {t(`about.story.timeline.${item.id}`)}
                  </p>
                </div>
              </li>
            ))}
